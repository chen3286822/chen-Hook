...
bool leftClickBlock(int x, int y)
{
	...
	int xpos = gOffset.x + 16 * y - gTitleOffset.x + 8;
	int ypos = gOffset.y + 16 * (gNumX - x - 1) - gTitleOffset.y + 8;
...
void updateMap(HDC hDc)
{
	...
	DWORD* clr = NULL;
	gethash(hDc, oriX, oriY, oriX + gNumY * gLength - 1, oriY + gNumX * gLength - 1, &clr);
	...
	for (int i = 0; i < gNumX; i++)
	{
		for (int j = 0; j < gNumY; j++)
		{
			if (gBlock[i*gNumY + j] != -1)
				continue;
			//((gNumY-j-1)*gLength+8)*gNumX*gLength+i*gLength+8
			switch (clr[(i * gLength + 9) * gNumY * gLength + j * gLength + 9] | 0xFF000000)
			{
			case 0xFF0000FF:
				gBlock[i*gNumY + j] = 1;
				break;
			case 0xFF008000:
				...
				break;
			case 0xFFFF0000:
				if ((clr[(i * gLength + 6) * gNumY * gLength + j * gLength + 8] | 0xFF000000) == 0xFF000000)
			...
			{
				if ((clr[(i * gLength + 15) * gNumY * gLength + j * gLength + 8] | 0xFF000000) == 0xFFFFFFFF)
					gBlock[i*gNumY + j] = -1;
				else
					gBlock[i*gNumY + j] = 0;
			}
...
bool checkDeep(Block point, vector<Block> unOpen, int result)
{
	...
	for (int i = 0; i < unOpen.size(); i++)
	{
		...
		vTarUnOpen.clear();
		//接触点在左右边
		if ((unOpen[i].x == point.x && unOpen[i].y == point.y - 1) ||
		...
		//接触点在上下边
		else if ((unOpen[i].x == point.x - 1 && unOpen[i].y == point.y) ||
